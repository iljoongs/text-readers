...
- 작업 단위가 끝나면 Claude Code가 커밋 메시지를 직접 작성해 커밋하고 원격(`origin`)에 푸시까지 진행한다. 별도 확인을 구하지 않는다(`doc/coding-convention.md`의 Git 커밋 규칙 참고). 커밋 메시지는 영어로 작성한다.
- 사용자의 지시(명령)로 개발 방향·규칙·사양이 바뀌는 경우, 그 내용을 관련 지시서(`CLAUDE.md` 또는 `doc/*.md`)에 즉시 반영한다.
- 각 작업을 수행한 뒤에는 이번 작업에서 참조한 지시서 파일이 무엇인지 사용자에게 알려준다.
